...
define Download/leosite.dat
  URL:=https://raw.githubusercontent.com/felix-fly/v2ray-adlist/master
  URL_FILE:=site.dat
  FILE:=site.dat
  HASH:=skip
endef
...
ifneq ($(CONFIG_V2RAY_EXCLUDE_ASSETS),y)
	...
	mv -f $(DL_DIR)/site.dat $(PKG_BUILD_DIR)/release/config/site.dat
...
ifneq ($(CONFIG_V2RAY_EXCLUDE_ASSETS),y)
	$(INSTALL_DATA) \
		$(PKG_BUILD_DIR)/release/config/site.dat \
		$(1)/usr/bin
endif
endef
...
$(eval $(call Download,leosite.dat))
endif
